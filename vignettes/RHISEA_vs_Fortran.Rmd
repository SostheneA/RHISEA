...
set.seed(42)
n_sim <- if (identical(Sys.getenv("NOT_CRAN"), "true")) 50 else 2
...
```{r simulation-loop, eval = (Sys.info()[['sysname']] == "Windows" && file.exists(system.file("bin/hisea.exe", package = "RHISEA")))}
exe_path <- system.file("bin/hisea.exe", package = "RHISEA")

work_dir <- tempdir()
old_dir <- setwd(work_dir)

on.exit({
  unlink(list.files(work_dir, pattern = "\\.(std|mix|out|xlsx|rda)$|^fort\\."), force = TRUE)
  setwd(old_dir) 
}, add = TRUE)
...
for (i in 1:n_sim) {
  ...
  system2(exe_path, wait = TRUE, stdout = FALSE)
...
```{r delete, echo=FALSE}
unlink(c("*.rda", "*.std", "*.mix", "*.ctr", "*.out", "*.xlsx", "fort.*","*.ctl", "*.exe"), force = TRUE)
