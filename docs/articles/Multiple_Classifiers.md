...
This vignette demonstrates how to **perform a comprehensive comparison
of multiple classification models** for mixed-stock analysis using the
**RHISEA** package.

RHISEA extends the classical HISEA mixed-stock analysis framework by
allowing users to incorporate a variety of modern classifiers and
evaluate their performance on the same dataset.

By running the provided **automated classifier comparison script**, you
will be able to:
...
- Summarize and visualize performance metrics such as accuracy and
  kappa,
...
The `baseline` dataset contains known stock samples labeled by
population. The `mixture` dataset contains unknown-origin samples to be
assigned to stocks.
...
This loop runs each classifier, performs the mixed-stock analysis using
RHISEA’s HISEA estimators, saves results in separate subfolders, and
...
```
    ...
    ## 1      ANN_ANALYSIS_20260316_174132    0.940 0.880
    ## 2      KNN_ANALYSIS_20260316_173620    0.940 0.880
    ## 3      LDA_ANALYSIS_20260316_171959    0.937 0.873
    ## 4 LDA_MASS_ANALYSIS_20260316_172032    0.937 0.873
    ## 5      MLR_ANALYSIS_20260316_174840    0.927 0.853
    ## 6       NB_ANALYSIS_20260316_174722    0.947 0.893
    ## 7      QDA_ANALYSIS_20260316_172103    0.937 0.873
    ## 8       RF_ANALYSIS_20260316_172923    0.967 0.933
    ## 9      SVM_ANALYSIS_20260316_173120    0.953 0.907
...
![Accuracy and Kappa for Each Model](figure/unnamed-chunk-5-1.png)
...
ggplot(performance_summary, aes(x = reorder(Model, Kappa), y = Kappa)) +
  ...
  coord_flip() +
  labs(
    title = "Classifier Kappa Comparison",
    x = "Classifier Model", y = "Kappa"
  ) +
  theme_minimal()
```

![Accuracy and Kappa for Each Model](figure/unnamed-chunk-5-2.png)
...
This approach provides a powerful way to evaluate and compare multiple
classification methods for mixed-stock analysis, enabling you to select
the most accurate and reliable model for your specific dataset.

The RHISEA package’s integration of modern classifiers with classical
HISEA estimation provides both flexibility and statistical rigor,
empowering fisheries and ecological researchers to improve stock
...
- Boxplots, trends over time, correlation between metrics, and heatmaps
  can be added similarly to further explore classifier performance.

- Saved CSVs and `.rda` files allow detailed post-hoc analyses and
  reproducibility.
